...
int dpsolution(int cur_pos)
{
    if(cur_pos>n) return 0;
    if(dp[cur_pos]!=-1) return dp[cur_pos];
    int k=0;
    for(int i=cur_pos+1;i<=min(cur_pos+2000,n);i++)
    {
        if(a[i][1]-a[cur_pos][1]>=abs(a[cur_pos][2]-a[i][2])+abs(a[cur_pos][3]-a[i][3]))
        {
            k=max(k,dpsolution(i));
        }
    }
    return dp[cur_pos]=k+1;
}
void solve()
{
    reset(dp,-1);
    ...
    fr1(1,i,n,1) in3(a[i][1],a[i][2],a[i][3]);


    /*for(int cu_pos=n;cu_pos>=0;cu_pos--)
    {
        dp[cu_pos]=1;

        for(int i=cu_pos+1;i<=min(n,cu_pos+1998);i++)
        {
            if(a[i][1]-a[cu_pos][1]>=abs(a[cu_pos][2]-a[i][2])+abs(a[cu_pos][3]-a[i][3]))
            {
                dp[cu_pos]=max(dp[cu_pos],dp[i]+1);
            }
        }
    }
    pf(dp[0]-1);*/
