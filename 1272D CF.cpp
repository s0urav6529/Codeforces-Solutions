...
void solve()
{
    ...
    fr1(1,i,n,1)
    {
        if(a[i]>a[i-1]) prefix[i]=prefix[i-1]+1;
        else prefix[i]=1;
    }
    rf1(n,i,1,1)
    {
    ...
    fr1(0,i,n+1,1)
    {
        if(i==0) umax(ans,suffix[i+1]);
        else if(i==n+1) umax(ans,prefix[i-1]);
        else if(a[i-1]<a[i+1]) umax(ans,prefix[i-1]+suffix[i+1]);
        else umax(ans,prefix[i-1]);
    }
    pf(ans);
    return;
    ///********************MAKE ALL THE INT LL****************************
