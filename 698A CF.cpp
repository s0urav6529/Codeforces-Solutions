...
int dp[3][102];
int solution(int i,int ac)
{
    if(i>n) return dp[ac][i]=0;
    if(dp[ac][i]!=-1) return dp[ac][i];
    int Min=101;
    if(i<=n&&a[i]==0&&(ac==0||ac==1||ac==2))
        Min=min(Min,1+solution(i+1,0));

    if(i<=n&&ac==0&& a[i]==1)
        Min=min(Min,solution(i+1,1));

    if(i<=n&&ac==0&& a[i]==2)
        Min=min(Min,solution(i+1,2));

    if(i<=n&&ac==0&& a[i]==3)
        Min=min(solution(i+1,1),solution(i+1,2));

    if(i<=n&&ac==1&& a[i]==1)
        Min=min(Min,1+solution(i+1,0));

    if(i<=n&&ac==1&&(a[i]==2||a[i]==3))
        Min=min(Min,solution(i+1,2));

    if(i<=n&&ac==2&& a[i]==2)
        Min=min(Min,1+solution(i+1,0));

    if(i<=n&&ac==2&&(a[i]==1||a[i]==3))
        Min=min(Min,solution(i+1,1));
