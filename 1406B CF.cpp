...
void solve()
{
    ll ans=imin,i=3,j=2,k=1,l=i+1,m=l+1;
    in(n);
    fr1(1,i,n,1) in(a[i]);
    for(i=3;i<=n-3;i++)
    {
        ll mx1=imin,mn1=imax,mx2=imin,mn2=imax,mx3=imin,mn3=imax,mx4=imin,mn4=imax;
        for(int p=j;p<i;p++)
        {
            if(a[i]<=0) mx1=max(mx1,a[i]),mn1=min(mn1,a[i]);
            else px1=max(px1,a[i]),pn1=min(pn1,a[i]);
        }
        for(int q=k;q<j;q++)
        {
            if(a[i]<=0) mx2=max(mx2,a[i]),mn2=min(mn2,a[i]);
            else px2=max(px2,a[i]),pn2=min(pn2,a[i]);
        }
        for(int r=l;r<n-1;r++)
        {
            if(a[i]<=0) mx3=max(mx3,a[i]),mn3=min(mn3,a[i]);
            else px3=max(px3,a[i]),pn3=min(pn3,a[i]);
        }
        for(int s=m;s<=n;s++)
        {
            if(a[i]<=0) mx4=max(mx4,a[i]),mn4=min(mn4,a[i]);
            else px4=max(px4,a[i]),pn4=min(pn4,a[i]);
        }
        j++,k++,l++,m++;
        ans=max(ans,
    }
