...
bool yes(ll mid)
{
    if(mid+1==sz(s)) return true;
    ll cc=c;
    for(ll i=mid;i<sz(s)-1;i++)
    {
        if(i==mid)
        {
            if(s[i]=='A'&&a<=cc) cc-=a;
            else if(s[i]=='B'&&b<=cc) cc-=b;
            else return false;
            continue;
        }
        if(s[i]=='A'&&s[i-1]=='B'&&a<=cc) cc-=a;
        else if(s[i]=='B'&&s[i-1]=='A'&&b<=cc) cc-=b;
        else if((s[i]=='B'&&s[i-1]=='B')||(s[i]=='A'&&s[i-1]=='A')) continue;
        else return false;
    }
    return true;
}
