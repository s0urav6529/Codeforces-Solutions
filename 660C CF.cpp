...
void solve()
{
    ...
    set<int>one;
    set<int>zero;
    int ans=0,ls,rs,Max=0,p=0;
    fr1(1,i,n,1)
    {
        in(a[i]);
        if(a[i]==1) p++;
        if(a[i]==0 || i==n) Max=max(Max,p),p=0;
    }
    if(k==0)
    {
        pfl(Max);
    ...
    fr1(1,i,n,1)
    {
        ...
        else
        {
            if(a[i]==0)
            {
                ...
                Max=i-z;
                zero.erase(z);
                if(*one.begin()<z && one.size()>0)
                {
                    while(*one.begin()<z&& one.size()>0) one.erase(*one.begin());
                }
            }
            else Max++;
        }
        if(Max>ans)
        {
            ls=max(1,min(*one.begin(),*zero.begin()));
            rs=i;
            //cout<<ls<<" "<<rs<<endl;
            ans=Max;
        }
