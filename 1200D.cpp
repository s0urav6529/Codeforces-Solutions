...
char a[2005][2005];
int rdp[2005][2005],cdp[2005][2005];;
void solution()
{
    int n,k,all=0;
    in2(n,k);
    fr1(1,i,n,1)
    {
        string s;
        cin>>s;
        fr1(1,j,n,1) a[i][j]=s[j-1];
    }
    fr1(1,i,n,1)
    {
        fr1(1,j,n,1)
        {
            if(a[i][j]=='B') rdp[i][j]=rdp[i][j-1]+1;
            else rdp[i][j]=rdp[i][j-1];
    ...
    fr1(1,i,n,1)
    {
        fr1(1,j,n,1)
        {
            ...
            if(a[j][i]=='B') cdp[j][i]=cdp[j-1][i]+1;
            else cdp[j][i]=cdp[j-1][i];
        }
        if(cdp[n][i]==0) all++;
    }
    k--;
    int allw=0,recw=0,ans=0;
    fr1(1,i,n,1)
    {
        fr1(1,j,n,1)
        {
