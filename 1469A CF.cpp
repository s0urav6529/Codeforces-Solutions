...
void solve()
{
    string s;
    in(s);
    int flag=0,pff=0,sp=0,sflag=0,other=0;
    for(int i=0;i<sz(s);i++)
    {
        if(s[i]=='(') pff=1;
        else if(s[i]==')' && pff==1) sp=1;
        if(pff==1 && (s[i]=='?' || s[i]=='/') && sp==0) flag++;
        else if(sp==1 && (s[i]=='/'|| s[i]=='?')) sflag++;
        else if(pff==0 && (s[i]=='/'|| s[i]=='?') ) other++;

    }
    if(flag%2==0 && sp==1 && (sflag+other)%2==0) pyes;
    else if((flag&1) && sp==1 && ((sflag-1)+other)%2==0) pyes;
    else if(sp==0)
    {
        other=0,sflag=0;
        int k=0,z=sz(s)-1;
