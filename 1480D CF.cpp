...
void solve()
{
    ...
    fr0(0,i,n,1)
    {
        if(s.size()==0 && p.size()==0) s.pb(a[i]);
        else if(p.size()==0)
        {
            if(s.back()==a[i]) p.pb(a[i]);
            else
            {
                if(a[i+1]==s.back()) s.pb(a[i]);
                else p.pb(a[i]);
            }
        }
        else
        {
            int sl=s.back(),pl=p.back();
            if(sl!=a[i] && pl!=a[i])
            {
                if(sl==a[i+1]) s.pb(a[i]);
                else p.pb(a[i]);
            }
            else if(sl==a[i] && pl==a[i]) continue;
            else if(sl==a[i]) p.pb(a[i]);
            else s.pb(a[i]);
        }
    }
    pf(s.size()+p.size());
