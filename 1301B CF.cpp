...
void solve()
{
    ll Min=imax,Max=imin;
    in(n);
    fr1(1,i,n,1) in(a[i]);
    fr1(1,i,n,1)
    {
        if(i>1&&a[i]==-1&&a[i-1]!=-1) Min=min(Min,a[i-1]),Max=max(Max,a[i-1]);
        if(i<n&&a[i]==-1&&a[i+1]!=-1) Min=min(Min,a[i+1]),Max=max(Max,a[i+1]);
    }
    ll mid=(Max+Min)/2,ans=0;
    fr1(1,i,n,1) if(a[i]==-1)a[i]=mid;
    fr1(1,i,n-1,1) ans=max(abs(a[i]-a[i+1]),ans);
    cout<<ans<<" "<<mid<<endl;
    return;
    ///********************MAKE ALL THE INT LL****************************
}
