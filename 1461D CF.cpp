...
vector<ll>a;
vector<ll>prefix;
void solution(int suru,int ses)
{
    possible_sum.insert(prefix[ses]-prefix[suru-1]);

    if(a[suru]==a[ses]) return;

    ll mid=(a[suru]+a[ses])/2;

    int indx=ub(a.begin()+suru,a.begin()+ses,mid)-a.begin();

    solution(suru,indx-1);
    solution(indx,ses);
}
void solve()
{
    ...
    fr1(1,i,q,1)
    {
        ll s;
        in(s);
        if(possible_sum.find(s)!=possible_sum.end())
        {
            pyes;
        }
        else pno;
