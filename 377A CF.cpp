...
{
    if(x<1 || y<1 || x>r || y>c || vis[x][y]==true || grid[x][y]=='#') return false;
    return true;
}
void dfs(int rr,int cc)
{
    ...
    fr1(0,i,3,1)
    {
        if(is_valid(rr+dx[i],cc+dy[i]))
        {
            ans.pb({rr+dx[i],cc+dy[i]});
            dfs(rr+dx[i],cc+dy[i]);
        }
    }
...
void solve()
{
    ...
    fr1(1,i,r,1)
        ...
        {
            cin>>grid[i][j];
            if(grid[i][j]=='.' && flag) x=i,y=j,flag=0;
        }
    ans.pb({x,y});
    dfs(x,y);
    rev(ans);
