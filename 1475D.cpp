...
void solution()
{
    ...
    for(int i=0;i<n;i++)
    {
        if(a[i].ss==2) t2.pb(a[i].ff);
        else o1.pb(a[i].ff);
    }
    sort(all(t2),greater<int>());
    sort(all(o1),greater<int>());

    int t2s=t2.size();
    int o1s=o1.size();
    vector<ll>prefix(t2s);
    for(int i=0;i<t2s;i++)
    {
        if(i>0) prefix[i]=prefix[i-1]+t2[i];
        else prefix[i]=t2[i];
    }
    int ans=1e9;
    int pos=lb(all(prefix),m)-prefix.begin();
    if(pos<t2s)
    {
        ans=(pos+1)*2;
    }
    ...
    for(int i=0;i<o1s;i++)
    {
        ...
        if(rem>0)
        {
            int pos=lb(all(prefix),rem)-prefix.begin();
            if(pos<t2s)
            {
                temp+=(pos+1)*2;
                umin(ans,temp);
