...
void solve()
{
    ...
    else
    {
        ...
        fr1(pid,i,sid,1)
        {
            if(s[i]=='1'||i==sid)
            {
                ll tempc1=(zero+2)*a+(zero-1)*b;
                ll tempc2=((zero-1)*2*b)+(zero*a);
                if(zero>1&&tempc1<tempc2) cost+=tempc1;
                else if(zero>1&&tempc1>=tempc2) cost+=tempc2;
                else if(zero==1)cost+=a;
                zero=0;
                one++;
            }
            if(s[i]=='0'||i==sid)
            {
                if(one>0) cost+=((one+1)*2*b)+(one*a);
                one=0;
                zero++;
            }
        }
