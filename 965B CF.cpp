...
void solve()
{
    ...
    fr1(1,i,n,1)
        fr1(1,j,n,1)
        {
            ll left=0,right=0,up=0,down=0,tot=0;
            if(a[i][j]=='.')
            {
                rf1(j-1,p,1,1)
                {
                ...
                if(left+right+1>=k) tot+=min4(left+right+2-k,left+1,right+1,k);
                if(up+down+1>=k) tot+=min4(down+up+2-k,up+1,down+1,k);
                if(tot>ans) r=i,c=j,ans=tot;
            }
        }
    see2(r,c);
    return;
