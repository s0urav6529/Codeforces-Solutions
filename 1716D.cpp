...
void run_case(){
    ...
    ll dp_prev[n+2],dp_cur[n+2],ans[n+2];

    for(int i=0;i<=n;i++) dp_prev[i]=dp_cur[i]=ans[i]=0;
    ...
    for(ll i=k;suru<=n;i++){
        ...
        for(ll j=suru;j<=n;j++){
            ...
            dp_cur[j]=(dp_prev[j-i]+dp_cur[j-i])%MOD;

            ans[j]=(ans[j]+dp_cur[j])%MOD;

        }

        for(ll j=0;j<=n;j++) dp_prev[j]=dp_cur[j];
