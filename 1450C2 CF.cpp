...
{
    in(n);
    vector<int>Xa(3,0),Oa(3,0);
    int opearation_placeX=0,opearation_placeO=0;
    fr1(1,i,n,1)
    {
        fr1(1,j,n,1)
        {
    ...
    if(min3(Xa[0],Xa[1],Xa[2])==Xa[1]) opearation_placeX=1;
    else if(min3(Xa[0],Xa[1],Xa[2])==Xa[2]) opearation_placeX=2;

    if(min3(Oa[0],Oa[1],Oa[2])==Oa[1]) opearation_placeO=1;
    else if(min3(Oa[0],Oa[1],Oa[2])==Oa[2]) opearation_placeO=2;

    fr1(1,i,n,1)
    {
        fr1(1,j,n,1)
        {
            if(a[i][j]=='X' && (i+j)%3==opearation_placeX) a[i][j]='O';
            else if(a[i][j]=='O' && (i+j)%3==opearation_placeO) a[i][j]='X';
            pf(a[i][j]);
        }
        nl;
    }
