...
int sol(ll i,ll t)
{
    ...
    if(t)
    {
        ...
        if(i<n&&a[i]==0)
            Min=min(Min,sol(i+1,t^1));

        if(i<n&&a[i]==1)
            Min=min(Min,1+sol(i+1,t^1));

        if(i+1<n&&a[i]==0&&a[i+1]==0)
            Min=min(Min,sol(i+2,t^1));

        if(i+1<n&&a[i]==0&&a[i+1]==1)
            Min=min(Min,1+sol(i+2,t^1));

        if(i+1<n&&a[i]==1&&a[i+1]==0)
            Min=min(Min,1+sol(i+2,t^1));

        if(i+1<n&&a[i]==1&&a[i+1]==1)
            Min=min(Min,2+sol(i+2,t^1));
    ...
    {
        int Min=imax;
        Min=min(sol(i+1,t^1),sol(i+2,t^1));
        return dp[i][t]=Min;
    }
}
void solve()
