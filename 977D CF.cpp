...
void solve()
{
    ...
    fr0(0,i,n,1)
    {
        if(s1.find(v[i]*2)!=s1.end())
        {
            s2.erase(v[i]);
        }
        if(v[i]%3==0&&s1.find(v[i]/3)!=s1.end())
        {
            s2.erase(v[i]);
        }
    }
    ...
    fr0(0,i,n-1,1)
    {
        if(s1.find(ans[i]*3)!=s1.end())
        {
            ans.pb(ans[i]*3); //pfg(ans[i]*3);
            continue;
        }
        if(s1.find(ans[i]/2)!=s1.end()&&ans[i]%2==0)
        {
            ans.pb(ans[i]/2); //pfg(ans[i]/2);
            continue;
        }

    }
    rev(ans);
    for(auto i:ans) pfg(i);
    return;
    ///********************MAKE ALL THE INT LL****************************
}
int main()
