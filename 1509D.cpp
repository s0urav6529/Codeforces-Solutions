...
void solution()
{
    ...
    int n;
    in4(n,a,b,c);
    int i=0,j=0,k=0,len=a.size();
    string ans="";
    while(i<len && j<len && k<len)
    {
        if(a[i]==b[j] && b[j]==c[k]) ans+=a[i],i++,j++,k++;
        else if(a[i]==b[j]) ans+=a[i],i++,j++;
        else if(b[j]==c[k]) ans+=b[j],j++,k++;
        else if(a[i]==c[k]) ans+=a[i],i++,k++;
    }
    //cout<<i<<" "<<j<<" "<<k<<endl;
    if((j>=i && i>=k)||(k>=i && i>=j))
    {
        for(int p=i;p<len;p++) ans+=a[p];
    }
    else if((i>=j && j>=k)||(k>=j && j>=i))
    {
        for(int p=j;p<len;p++) ans+=b[p];
    }
    else if((i>=k && k>=j)||(j>=k && k>=i))
    {
        for(int p=k;p<len;p++) ans+=c[p];
    }
    cout<<ans<<endl;
    return;
}
