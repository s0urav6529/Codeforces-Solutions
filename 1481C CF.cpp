...
void solve()
{
    int n,p;
    in2(n,p);
    int ini[n+5],fin[n+5],ptr[p+5];
    map<int,vector<int>>mp;
    fr1(1,i,n,1) in(ini[i]);
    fr1(1,i,n,1)
    {
        in(fin[i]);
        if(fin[i]!=ini[i]) mp[fin[i]].pb(i);
    }
    fr1(1,i,p,1) in(ptr[i]);
    int indx=0;
    if(mp[ptr[p]].size())
    {
        indx=mp[ptr[p]].back();
    ...
    fr1(1,i,p,1)
    {
        ...
        else if(mp[ptr[i]].size())
        {
            ans.pb(mp[ptr[i]].front());
            ini[mp[ptr[i]].front()]=ptr[i];
            mp[ptr[i]].erase(mp[ptr[i]].begin());
        }
    }
    fr1(1,i,n,1)
    {
