...
int op=0;
void dfs(int src,int parent,int even,int odd,int lvl)
{
    if(lvl&1)
    {
        int curpos=(init[src]+odd)%2;
        if(curpos!=goal[src])
        {
            op++;
            odd++;
    ...
    for(auto child:ad[src])
    {
        if(child!=parent) dfs(child,src,even,odd,lvl+1);
    }
    return;
}
void solution()
{
    ...
    dfs(1,0,0,0,0); ///root ,parent, even lvl op , odd lvl op , level
    cout<<op<<endl;
