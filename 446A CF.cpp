...
void solution()
{
    ...
    {
        if(i==1) inc[i]=1;
        else if(a[i]>a[i-1]) inc[i]=inc[i-1]+1;
        else inc[i]=1;
    }
    for(int i=n;i>=1;i--)
    {
        if(i==n) dec[i]=1;
        else if(a[i]<a[i+1]) dec[i]=dec[i+1]+1;
        else dec[i]=1;
    }
    int ans=1;
    for(int i=1;i<=n;i++)
    {
        ll temp;
        if(i==1) umax(ans,dec[i+1]+1);
        else if(i==n) umax(ans,inc[i-1]+1);
        else
        {
            umax(ans,inc[i]+dec[i]-1);
            temp=a[i-1]+1;
            int x=inc[i-1]+1;
            if(temp<a[i+1]) x+=dec[i+1];
            umax(ans,x);
            temp=a[i+1]-1;
            x=dec[i+1]+1;
            if(temp>a[i-1]) x+=inc[i-1];
            umax(ans,x);
        }
    }
    cout<<ans;
