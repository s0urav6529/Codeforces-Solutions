...
vector<pii>adj[105];
bool vis[105]={false};
bool dfs(int source,int desti,int color)
{
    vis[source]=true;

    if(source==desti) return true;  ///base condition

    for(pii child: adj[source])
    {
        if(child.ss==color && vis[child.ff]==false)
        {
            if(dfs(child.ff,desti,color)) return true;
        }
    }
    return false;
}
