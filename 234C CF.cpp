...
void solve()
{
    ...
    fr1(1,i,n,1)
    {
        if(a[i]==0)
        {
            zero[i]=zero[i-1]+1; pos[i]=pos[i-1]; neg[i]=neg[i-1];
        }
        else if(a[i]>0)
        {
            pos[i]=pos[i-1]+1; zero[i]=zero[i-1]; neg[i]=neg[i-1];
        }
        else
        {
            neg[i]=neg[i-1]+1; zero[i]=zero[i-1]; pos[i]=pos[i-1];
        }
    }
    int Min=imax;
    fr1(1,i,n-1,1)
    {
        umin(Min,zero[n]+(neg[n]-neg[i])+pos[i]);
    }
    ...
    rf1(n,i,2,1)
    {
        umin(Min,zero[1]+(pos[1]-pos[i])+neg[i]);
    }
    pf(Min);
    return;
    ///********************MAKE ALL THE INT LL****************************
