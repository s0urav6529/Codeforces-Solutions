...
void solve()
{
    ...
    rf1(n,i,1,1)
    {
        ...
        else if(a[i]==3)
        {
            ...
            else if(index[2]<two.size())
            {
                int C=two[index[2]].ss;
            ...
            else if(index[1]< one.size())
            {
                int C=one[index[1]].ss;
    ...
    pfl(one.size()+two.size()+three.size());
    for(auto i: one) pf2(i.ff,i.ss);
    for(auto i: two) pf2(i.ff,i.ss);
    for(auto i: three) pf2(i.ff,i.ss);
    return;
    ///********************MAKE ALL THE INT LL****************************
}
int main()
