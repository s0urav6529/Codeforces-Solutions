...
void solve()
{
    ...
    ll ans=0;
    int r0=max(zero-n/3,0);
    int r1=max(one-n/3,0);
    int r2=max(two-n/3,0);
    while(zero<n/3)
    {
        int m=min(abs(n/3-zero),r2);
        ans+=(m*1);
        zero+=m;
        r2-=m;
        m=min(abs(n/3-zero),r1);
        ans+=(m*2);
        zero+=m;
        r1-=m;
    ...
    while(one<n/3)
    {
        int m=min(abs(n/3-one),r0);
        ans+=(m*1);
        one+=m;
        r0-=m;
        m=min(abs(n/3-one),r2);
        ans+=(m*2);
        one+=m;
        r2-=m;
    ...
    while(two<n/3)
    {
        int m=min(abs(n/3-two),r1);
        ans+=(m*1);
        two+=m;
        r1-=m;
        m=min(abs(n/3-two),r0);
        ans+=(m*2);
        two+=m;
        r0-=m;
