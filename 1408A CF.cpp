...
void solve()
{
    ...
    fr1(1,i,n,1)
    {
        if(i==1) pfg(a[i]),bef=a[i],f=a[i];
        else if(i==n)
        {
            if(bef!=a[i]&& f!=a[i]) pfg(a[i]);
            else if(bef!=b[i]&&f!=b[i]) pfg(b[i]);
            else if(bef!=c[i]&&f!=c[i]) pfg(c[i]);
        }
        else
        {
            if(bef!=a[i]) pfg(a[i]),bef=a[i];
            else if(bef!=b[i]) pfg(b[i]),bef=b[i];
            else if(bef!=c[i]) pfg(c[i]),bef=c[i];
        }
    }
    pfl("");
    return;
