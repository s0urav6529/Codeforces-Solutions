...
void solve()
{
    ...
    fr1(1,i,m,1)
    {
        ...
        while(1)
        {
            ...
            psize=x.size();
            for(string ch:adj[x[0]-'a'])
            {
                if(ch!="-1")
                {
                    string d=x.substr(1,x.size()-1);
                    x=ch+d;
                    ch="-1";
                }
            }
            if(psize==x.size()) break;
        }
    }
    pfl(st.size());
    for(auto i:st) pfg(i);
