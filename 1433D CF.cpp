...
void solve()
{
    ...
    pyes;
    sortv(segs);
    int cur=-100,val=0,mini=imax,num;
    fr0(0,i,n,1)
    {
        if(cur!=segs[i])
        {
    ...
    {
        mini=min(mini,val);
        if(mini==val) num=segs[n-1];
    }
    fr0(0,i,n,1)
    {
        if(num!=v[i]) val=i+1;  ///position of distinct value
        else if(num==v[i]) cur=i+1; ///position of min occurence value
    }
    fr0(0,i,n,1)
    {
        if(num!=v[i])
