...
void solve()
{
    in(n);
    ll a[n+5][n+5];
    ll m_diagornal[n+5][n+5],s_diagonal[n+5][n+5];
    fr1(1,i,n,1) fr1(1,j,n,1) in(a[i][j]);
    fr1(1,i,n,1)
    {
        fr1(1,j,n,1)
        {
           if(i==1 || j==1) m_diagornal[i][j]=a[i][j];
           else m_diagornal[i][j]=a[i][j]+m_diagornal[i-1][j-1];

           if(i==1 || j==n) s_diagonal[i][j]=a[i][j];
           else s_diagonal[i][j]=a[i][j]+s_diagonal[i-1][j+1];
        }
    }
    int x1,y1,x2,y2;
    ll mx=imin;
    fr1(1,i,n,1)
    {
        ...
        {
            ll ans;
            if(i==j) ans=(m_diagornal[n][n]+s_diagonal[][1]-a[i][j]);

            else if(j<i) ans=(m_diagornal[n][n-(i-j)]+s_diagonal[][]-a[i][j]);

            else ans=(m_diagornal[n-(j-i)][n]+s_diagonal[][]-a[i][j]);
            if(ans>mx)
            {
                mx=ans;
                x1=i,y1=j;
