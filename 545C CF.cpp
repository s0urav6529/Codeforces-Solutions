...
void solve()
{
    ...
    fr0(0,i,n,1) in2(v[i].ff,v[i].ss);
    int prev=v[0].ff,next,ans=min(2,n);
    fr0(1,i,n-1,1)
    {
        next=v[i+1].ff;
        if(prev<v[i].ff-v[i].ss)///left falling
        {
            ans++;
            prev=v[i].ff;
        }
        else if(next>v[i].ff+v[i].ss)///right falling
        {
            ans++;
            prev=v[i].ff+v[i].ss;
        }
        else prev=v[i].ff;///not falling
    }
    pf(ans);
    return;
    ///********************MAKE ALL THE INT LL****************************
