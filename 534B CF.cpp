...
#define fr1(p,i,k,z) for (int i=p ; i<=k ; i+=z)
#define rf1(p,i,k,z) for (int i=p ; i>=k ; i-=z)
#define fr0(p,i,k,z) for (int i=p ; i<k ; i+=z)
#define rf0(p,i,k,z) for (int i=p-1 ; i>=k ; i-=z)
#define dcfr1(p,i,k,f,e,z) for (int i=p ; i<=k&&f>e ; i+=z)
#define dcrf1(p,i,k,f,e,z) for (int i=p ; i>=k&&f>e ; i-=z)
#define dcfr0(p,i,k,f,e,z) for (int i=p ; i<k&&f>e ; i+=z)
#define dcrf0(p,i,k,f,e,z) for (int i=p-1 ; i>=k&&f>e ; i-=z)
...
int solution(int pos,int val)
{
    ...
    if(done[pos][val]) return dp[pos][val];

    done[pos][val]=true;

    dp[pos][val]=imin;

    if(pos==1) return dp[pos][val]=val+solution(pos+1,val);
    else
    {
        fr1(1,j,d,1)
        {
            if(val-j>=0)
                dp[pos][val]=max(dp[pos][val],val-j+solution(pos+1,val-j));
            dp[pos][val]=max(dp[pos][val],val+j+solution(pos+1,val+j));
        }
        dp[pos][val]=max(dp[pos][val],val+solution(pos+1,val));
        return dp[pos][val];
    }
}
void solve()
