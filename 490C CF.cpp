...
int to_power(ll base,ll Pow,ll M)
{
    ll ans=1;
    while(Pow)
    {
...
{
    in3(s,a,b);
    vector<int>first_string,second_string;
    ll number=0;
    for(int i=0;i<sz(s)-1;i++)
    {
        ...
        number+=(s[i]-'0');
        number%=a;
        if(number==0 && s[i+1]!='0') first_string.pb(i);
    }
    //for(auto i:first_string) cout<<i<<" ";
    ll Reminder=0;
    for(int i=sz(s)-1;i>=0;i--)
    {
        number=to_power(10,sz(s)-1-i,b);
        number*=(s[i]-'0');
        number%=b;
        Reminder+=number;
        Reminder%=b;
        //cout<<Reminder<<" ";
        if(Reminder==0) second_string.pb(i);
    }
    //for(auto i:second_string) cout<<i<<" ";
    sortv(second_string);
    for(auto i:first_string)
    {
        if(binary_search(second_string.begin(),second_string.end(),i+1))
        {
            pyes;
            for(int j=0;j<=i;j++) pf(s[j]);
            nl;
            for(int j=i+1;j<sz(s);j++) pf(s[j]);
            return;
        }
    }
    pno;
