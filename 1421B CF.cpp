...
void solve()
{
    ...
    int right=a[1][2]-'0',down=a[2][1]-'0',left=a[n][n-1]-'0',up=a[n-1][n]-'0';
    if(right+down==2 && left+up ==2)
    {
        pfl(2);
        pf2(1,2);
        pf2(2,1);
    }
    else if(right+down==0 && left+up ==0)
    {
        pfl(2);
        pf2(1,2);
        pf2(2,1);
    }
    else if(right+down==1 && left+up==0)
    {
        pfl(1);
        if(right==0) pf2(1,2);
        else if(down==0) pf2(2,1);
    }
    else if(right+down==0 && left+up== 1)
    {
        pfl(1);
        if(left==0) pf2(n,n-1);
        else if(up==0) pf2(n-1,n);
    }
    else if(right+down==1 && up+left ==1)
    {
        pfl(2);
        if(right) pf2(1,2);
        else if(down) pf2(2,1);
        if(left==0) pf2(n,n-1);
        else if(up==0) pf2(n-1,n);
    }
    else if(right+down==1 && left+up==2)
    {
        pfl(1);
        if(right) pf2(1,2);
        else if(down) pf2(2,1);
    }
    else if(right+down==2 && left+up==1)
    {
        pfl(1);
        if(left) pf2(n,n-1);
        else if(up) pf2(n-1,n);
    }
    else pfl(0);
    return;
    ///********************MAKE ALL THE INT LL****************************
