...
void solve()
{
    ...
    int prefix[n+5],prefix_sum[n+5],suffix[n+5],suffix_sum[n+5],power[n+5],power_sum[n+5];

    prefix_sum[0]=num(s[0]),suffix_sum[n]=num(s[n]);

    prefix[0]=num(s[0]),suffix[n]=num(s[n]);

    for(int i=1;i<=n;i++)
    {
        prefix[i]=addmod(mulmod(prefix[i-1],10),num(s[i]));

        prefix_sum[i]=addmod(prefix[i],prefix_sum[i-1]);
    }
    ll p=10;
    for(int i=n-1;i>=0;i--)
    {
        suffix[i]=addmod(mulmod(num(s[i]),p),suffix[i+1]);

        suffix_sum[i]=addmod(suffix_sum[i+1],suffix[i]);

        p=mulmod(p,10);
    }

    power[0]=1,power_sum[0]=0;

    for(int i=1;i<=n;i++)
    {
        power[i]=mulmod(power[i-1],10);

        power_sum[i]=addmod(power_sum[i-1],power[i]);
    }
    ll ans=0;

    ans=addmod(ans,suffix_sum[1]);
    ans=addmod(ans,prefix_sum[n-1]);

    for(int i=1;i<n;i++)
    {
        ll temp=mulmod(prefix[i-1],power_sum[n-i]);
        temp=addmod(temp,suffix_sum[i+1]);
