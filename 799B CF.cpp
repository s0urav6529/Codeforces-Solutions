...
{
    in(n);
    ll p[n+1],fc[n+1],bc[n+1];
    set<ll> s13,s12,s11,s23,s22,s33;
    fr1(1,i,n,1) in(p[i]);
    fr1(1,i,n,1) in(fc[i]);
    fr1(1,i,n,1) in(bc[i]);
    fr1(1,i,n,1)
    {
        if(fc[i]==1&&bc[i]==1) s11.insert(p[i]);
        else if(fc[i]==1&&bc[i]==2) s12.insert(p[i]);
        else if(fc[i]==1&&bc[i]==3) s13.insert(p[i]);
        else if(fc[i]==2&&bc[i]==2) s22.insert(p[i]);
        else if(fc[i]==2&&bc[i]==1) s12.insert(p[i]);
        else if(fc[i]==2&&bc[i]==3) s23.insert(p[i]);
        else if(fc[i]==3&&bc[i]==3) s33.insert(p[i]);
        else if(fc[i]==3&&bc[i]==1) s13.insert(p[i]);
        else if(fc[i]==3&&bc[i]==2) s23.insert(p[i]);
    }
    in(m);
    fr1(1,i,m,1)
    {
        ...
        if(x==3)
        {
            ll p=lmax,q=lmax,r=lmax;
            if(!s13.empty()) p=*s13.begin();
            if(!s23.empty()) q=*s23.begin();
            if(!s33.empty()) r=*s33.begin();
            ll mn=min3(p,q,r);
            if(mn==lmax)
            {
                pfg(-1);
            ...
            pfg(mn);
            if(mn==p) s13.erase(p);
            else if(mn==q) s23.erase(q);
            else if(mn==r) s33.erase(r);
        }
        else if(x==2)
        {
            ll p=lmax,q=lmax,r=lmax;
            if(!s12.empty()) p=*s12.begin();
            if(!s23.empty()) q=*s23.begin();
            if(!s22.empty()) r=*s22.begin();
            ll mn=min3(p,q,r);
            if(mn==lmax)
            {
                pfg(-1);
            ...
            else if(mn==r) s22.erase(r);
        }
        else
        {
            ll p=lmax,q=lmax,r=lmax;
            if(!s13.empty()) p=*s13.begin();
            if(!s12.empty()) q=*s12.begin();
            if(!s11.empty()) r=*s11.begin();
            ll mn=min3(p,q,r);
            if(mn==lmax)
            {
                pfg(-1);
            ...
            pfg(mn);
            if(mn==p) s13.erase(p);
            else if(mn==q) s12.erase(q);
            else if(mn==r) s11.erase(r);
        }
    }
    return;
    ///********************MAKE ALL THE INT LL****************************
