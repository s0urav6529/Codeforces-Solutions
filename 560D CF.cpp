...
string a,b;
bool solve(int al,int ar,int bl,int br)
{
    int i,j;
    for(i=al,j=bl;i<=ar && j<=br;i++,j++)
        if(a[i]!=b[j]) break;

    if(i>ar) return true; ///all are euqal.
    int len=ar-al+1;
    if(len&1) return false;
    len/=2;

    bool q1=(solve(al,al+len-1,bl,bl+len-1) && solve(al+len,ar,bl+len,br)) ;

    if(q1) return q1;

    bool q2=(solve(al,al+len-1,bl+len,br) && solve(al+len,ar,bl,bl+len-1)) ;
...
{
    cin>>a>>b;
    if(solve(0,a.size()-1,0,b.size()-1)) pyes;
    else pno;
    return;
}
int main()
