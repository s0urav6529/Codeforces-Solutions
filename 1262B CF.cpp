...
void solve()
{
    int Max=0,j=1,flag=1;
    in(n);
    int ans[n+1];
    int Maxi[n+1];
    bool Fill[n+1];
    int gap[n+1];
    fr1(1,i,n,1) Fill[i]=false;
    fr1(1,i,n,1)
    {
        int x;
        in(x);
    ...
    fr1(1,i,n,1)
    {
        if(Fill[Maxi[i]]==false) ans[i]=Maxi[i],Fill[Maxi[i]]=true;
        else ans[i]=0;
    }
    fr1(1,i,n,1)
    {
        if(Fill[i]==false) gap[j++]=i;
    }
    sort(gap+1,gap+j);
    j=1;
    fr1(1,i,n,1)
    {
        if(Fill[Maxi[i]]==true && ans[i]==0)
        {
            if(gap[j]<Maxi[i] )
            {
                ans[i]=gap[j];
