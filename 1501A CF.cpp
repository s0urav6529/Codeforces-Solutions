...
void solution()
{
    int n;
    cin>>n;
    int a[n+5][2],b[n+5][2],l[n+5],gap[n+5];
    for(int i=1;i<=n;i++)
    {
        cin>>a[i][1]>>a[i][2];
        b[i][1]=a[i][1],b[i][2]=a[i][2];
        if(i==1) gap[i]=0;
        else  gap[i]=a[i][1]-a[i-1][2];
    }
    for(int i=1;i<=n;i++) cin>>l[i];
    for(int i=1;i<=n;i++)
    {
        if(i==1) a[i][1]+=l[i];
        else if(a[i][1]<=a[i-1][2])a[i][1]=a[i-1][2]+l[i]+gap[i];
        else if(b[i-1][2]==a[i-1][2]) a[i][1]+=l[i];
        else a[i][1]+=l[i]+(a[i-1][2]-b[i-1][2]);
        if(a[i][2]<=a[i][1])
        {
            a[i][2]=a[i][1]+1;
        }
