...
void solve()
{
    int podd=imax,peven=-1,nodd=imin;
    ll sum=0;
    in(n);
    fr1(1,i,n,1)
    {
        ...
        {
            sum+=x;
            if(x&1) podd=min(podd,x);
        }
        else if(abs(x)&1) nodd=max(nodd,x);
    }
    //pf3(podd,peven,nodd);
    if(sum&1) pf(sum);
    else if(sum==0) pf(nodd);
    else
    {
        if(podd!=imax&&nodd!=imin)
        {
            pf(max(sum-podd,sum+nodd));
        }
        else if(podd!=imax)
        {
            pf(sum-podd);
